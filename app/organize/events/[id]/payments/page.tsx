...
export default function EventPaymentsPage({ params }) {
  ...
  const downloadAllBookings = async () => {
    ...
    try {
      ...
      doc.setFontSize(14);
      doc.text("All Bookings Payment Report", 20, 30);
      ...
      doc.text(`Event: Bike Riding`, 20, 60);
      ...
      doc.text(`Total Due: KSh ${totalDue.toLocaleString()}`, 20, 90);
      doc.text(
      ...
      doc.text("Booking ID", 22, yPosition);
      doc.text("Customer", 55, yPosition);
      doc.text("Date", 95, yPosition);
      doc.text("Total", 120, yPosition);
      doc.text("Paid", 140, yPosition);
      ...
      bookings.forEach((booking, index) => {
        ...
        doc.setTextColor(0, 0, 0);
        doc.text(booking.id.slice(0, 10), 22, yPosition); // shorter ID
        doc.text(booking.userName.slice(0, 20), 55, yPosition);
        doc.text(booking.bookingDate.toLocaleDateString(), 95, yPosition);
        doc.text(`${booking.totalAmount.toLocaleString()}`, 120, yPosition);
        doc.text(`${booking.amountPaid.toLocaleString()}`, 140, yPosition);
        ...
        doc.text(booking.paymentStatus.toUpperCase(), 180, yPosition);
      ...
      doc.setFontSize(8);
      doc.text("Generated by Kenya Trails Booking System", 105, 285, {
        align: "center",
      });

      // Save
      doc.save(
        `Kenya_Trails_${event.title} _Bookings_${
            ...
            <TabsContent value="list">
              {filteredBookings().length === 0 ? (
                ...
                <div className="space-y-6">
                  <Card className="bg-white  shadow-lg hover:shadow-xl   transition-shadow duration-300 w-1/2 md:w-1/6">
                    <div className="text-center items-center p-4 ">
                      <Button
                        onClick={downloadAllBookings}
                        disabled={isDownloading}
                        className="w-full bg-green-600 hover:bg-green-700 text-white py-3 text-sm font-semibold transition-colors duration-200"
                        size="lg">
                        {isDownloading ? (
                          <>
