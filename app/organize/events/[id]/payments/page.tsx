...
import { downloadBookingReceipt } from "@/lib/modern-receipt-generator";
import {
  downloadAllBookings,
  downloadBookingsAsExcel,
} from "@/lib/downloads/bookings";
...
export default function EventPaymentsPage({ params }) {
  ...
  const totalWithdrawals = withdrawals.reduce(
    (sum, b) => sum + (b.status === "completed" ? b.amount || 0 : 0),
    0
  );
  ...
  const handleWithdrawalRequest = async (e) => {
    ...
    try {
      ...
      const platformFee = Math.max(withdrawAmount * 0.006, 10);
  ...
  if (loading) {
    return (
      <div className="md:px-12 mx-auto flex flex-col items-center justify-center min-h-screen">
  ...
  if (!event) {
    return (
      <div className="md:px-12 mx-auto px-4 py-16 flex flex-col items-center">
  ...
  return (
    <div className="md:px-12 mx-auto px-4 py-8 w-6/8">
      {/* Header */}
      <div className="mb-8">
        <Button
          variant="ghost"
            ...
            <TabsContent value="list">
              {filteredBookings().length === 0 ? (
                ...
                <div className="space-y-6">
                  <Card className="bg-white shadow-lg hover:shadow-xl transition-shadow duration-300 w-full md:w-1/2">
                    <div className="p-4">
                      <div className="space-y-4">
                        ...
                        <Button
                          onClick={() => {
                            console.log("am to download as");
                            downloadFormat === "pdf"
                              ? downloadAllBookings(bookings, event)
                              : downloadBookingsAsExcel(
                                  filteredBookings,
                                  event,
                                  bookings,
                          ...
                          disabled={
                            isDownloading || filteredBookings().length === 0
                          }
                        ...
                        {filteredBookings().length === 0 && (
                          <p className="text-sm text-gray-500 text-center">
                            No bookings available for download
                          </p>
                        )}
                      </div>
                    </div>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-gray-50 p-3 rounded-md">
                <p className="text-sm text-gray-600">
                  ...
                  within 2-4 hours. A confirmation email will be sent once the
